...
router.post('/', (req, res) => {
  ...
  pool
    ...
    .then((result) => {
      console.log('Result is:', result.rows[0]); //ID IS HERE!
...
router.delete('/:id', (req, res) => {
  ...
  const queryText = `DELETE FROM survey_table WHERE id = $1 RETURNING "user_id";`;
  pool
    .query(queryText, [idToDelete])
    .then((result) => {
       console.log('survey:Delete Result is:', result.rows[0]);
       res.send(result.rows[0]);
    })
    .catch((error) => {
      console.log(`Error making database query ${queryText}`, error);
